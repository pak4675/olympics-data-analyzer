...
def medal(df):
    ...
    medal_telly["total"] = medal_telly["Gold"] + medal_telly["Bronze"] + medal_telly["Silver"]
    return medal_telly
...
def fetch_medal_tally(df, year, country):
    ...
    if flag == 1:
        x = temp_df.loc[:, ~df.columns.duplicated()]  # Removes duplicates safely

        x = x.groupby('Year').sum()[['Gold', 'Silver', 'Bronze']].sort_values('Year').reset_index()
    else:
        x = temp_df.loc[:, ~df.columns.duplicated()]  # Removes duplicates safely
...
def country_medal_telly(df, country):
    temp_df = df.dropna(subset=['Medal'])
    temp_df.drop_duplicates(subset=['Team', 'NOC', 'Games', 'Year', 'City', 'Sport', 'Event', 'Medal'], inplace=True)

    new_df = temp_df[temp_df['region'] == country]
    y = new_df.groupby('Year').count()['Medal'].reset_index()
    return y
